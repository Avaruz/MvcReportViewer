...
namespace MvcReportViewer
{
  internal class ReportViewerParametersParser
  {
    ...
    public ReportViewerParameters Parse(NameValueCollection queryString)
    {
      ...
      foreach (var key in queryString.AllKeys)
      {
        ...
        else if (key.EqualsIgnoreCase(UriParameters.DataSourceCredentials))
        {
          ...
          parameters.DataSourceCredentials = JsonSerializer.Deserialize<DataSourceCredentials[]>(json);
        }
      ...
      return string.IsNullOrEmpty(parameters.ReportPath) ? throw new MvcReportViewerException("Report is not specified.") : parameters;
    }
    ...
    private static Tuple<string, bool> ParseKey(string key)
    {
      ...
      bool isVisible;
      return parts.Length != 2 || !bool.TryParse(parts[1], out isVisible)
              ? new Tuple<string, bool>(key, true)
              : new Tuple<string, bool>(parts[0], isVisible);
    }
